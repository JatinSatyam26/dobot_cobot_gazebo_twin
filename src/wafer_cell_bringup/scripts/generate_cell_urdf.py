...
def main(telemetry=False):
    """telemetry=True writes cell_telemetry.urdf: the same cell without the belt carriage (no
    carriage link, no belt_travel joint, no nest weld) for cell.launch.py telemetry:=true."""
    cell = ET.Element('robot', {'name': 'wafer_cell'})
    ET.SubElement(cell, 'link', {'name': 'world'})

    for name, pkg, xf, prefix, xyz, rpy in ROBOTS:
        root = expand(pkg, xf, prefix, ('carriage:=false',) if (telemetry and name == 'belt') else ())
        base = None
        for child in root:
    ...
    present_links = {l.get('name') for l in cell.findall('link')}
    present_joints = {j.get('name') for j in cell.findall('joint')}
    for rj in list(rc.findall('joint')):                 # no controller entry for a joint the variant lacks
        if rj.get('name') not in present_joints:
            rc.remove(rj); print(f"  ros2_control: dropped {rj.get('name')} (not in this variant)")
    for carrier, link in GRASP_LINKS.items():
        if link not in present_links:
            print(f'  grasp {carrier}: skipped, link {link} not in this variant'); continue
    ...
    src = Path(__file__).resolve().parents[1] / 'urdf' / ('cell_telemetry.urdf' if telemetry else 'cell.urdf')
    hdr = ('<?xml version="1.0"?>\n<!-- Copyright (c) 2026 Jatin Satyam. SPDX-License-Identifier: Apache-2.0 -->\n<!-- GENERATED by '
